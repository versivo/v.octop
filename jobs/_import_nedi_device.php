...
// verify token
$token = addslashes(stripslashes($argv[1]))
if ($token == $token_import)
{
	// get data from nedi
	mysqli_select_db($db_nedi, 'nedi');
	$sql_device = "SELECT * FROM `devices` ORDER BY `id`";
	$res_device = mysqli_query($db_nedi, $sql_device);
	while($out_device = mysqli_fetch_array($res_device))
	{
